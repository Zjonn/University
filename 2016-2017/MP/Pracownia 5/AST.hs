...
instance Show (Expr p) where
  ...
  showsPrec p (EUnary _ op e) =
    showParen (p > p') (showString "un(" .shows op . showChar ',' .
      showsPrec p' e . showChar ')')
    where
  ...
  showsPrec p (EBinary _ op e1 e2) =
    showParen (p > p')
      (showString "bin(" .showsPrec leftPrec e1 . showString "," .
      shows op . showString "," . showsPrec rightPrec e2.showChar ')')
    where
  ...
  showsPrec p (ELet _ x e1 e2) =
    showParen (p > 0)
      ...
      . showString "," . showsPrec 0 e2 . showChar ')')

  showsPrec p (EIf _ e1 e2 e3) =
    showParen (p > 0)
      (showString "if(" . showsPrec 0 e1 . showString ","
      . showsPrec 0 e2 . showElse e3 . showString ")")
    where
      showElse (EIf _ e1 e2 e3) =
        showString "," . showsPrec 0 e1 . showString ","
        . showsPrec 0 e2 . showElse e3
      showElse e = showString "," . showsPrec 0 e

  showsPrec p (EApp _ ff e) =
    showParen (p > 100)
      (showString "app(" . showString ff .
      showString "," . showsPrec 101 e . showString ")")

  showsPrec _ (EUnit _) = showString "()"

  showsPrec _ (EPair _ e1 e2) =
    showString "pair(" . showsPrec 0 e1 .
  ...
  showsPrec p (EFst _ e) =
    showParen (p > 100) (showString "fst(" . showsPrec 101 e.showString ")")

  showsPrec p (ESnd _ e) =
    showParen (p > 100) (showString "snd(" . showsPrec 101 e.showString ")")
